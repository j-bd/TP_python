...
path = "/home/latitude/Documents/Yotta/yotta_exs/yotta_p1/old-version/data/raw"
...
#    plt.figure()
#    df_filter.plot.bar(x=var)

#for var in cat_variables:
#    plt.figure()
#    agg = df_filter.groupby(var).agg({'SUBS_NUM': ('sum')})
#    ax = sns.barplot(x=agg/agg.sum, hue=agg[var])
#    ax.set(ylabel="Percent")
#
#for var in cat_variables:
#    plt.figure()
#    agg = df_filter.groupby(var).agg({'SUBS_NUM': ('sum')})
#    sns.barplot(x=df_filter[var], y=df_filter[var], hue=df_filter['SUBSCRIPTION']
#
#    )
...
for var in cat_variables:
    plt.figure()
    graph = sns.countplot(x=df_filter[var], hue=df_filter['SUBSCRIPTION'])
